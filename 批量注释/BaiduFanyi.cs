...
namespace 批量注释
{
    public class BaiduFanyi
    {
        ...
        {
            Console.WriteLine($"正在翻译：{q}");
            Thread.Sleep(1500);
            //可以直接到百度翻译API的官网申请
            //一定要去申请，不然程序的翻译功能不能使用
            string appId = ConfigurationManager.AppSettings["bdApiKey"].ToString();
            string password = ConfigurationManager.AppSettings["bdApiPwd"].ToString();
            if (string.IsNullOrEmpty(appId)||string.IsNullOrEmpty(password))
            {
               // Console.WriteLine("请配置百度翻译api的key和密匙");
                return new TranslationResult
                {
                    Error_code= "请配置百度翻译api的key和密匙"
                };
            }
